...
const Slide = () => {
    const { total, setTotal, remain, setRemain, number, setNumber, setPlay } = useGame((data) => data);
    const [numberSet, setNumberSet] = useState(() => +localStorage.getItem("number") || 100);
    ...
    const handleTooltip = () => {
        ...
        if (rect.width + progress.offsetWidth >= window.innerWidth) {
            const left = rect.width + progress.offsetWidth - 7 - window.innerWidth;
            tooltip.style.left = `calc(50% - ${left}px)`;
            tooltip.dataset.left = left;
            tooltip.firstElementChild.style.left = `calc(50% + ${left}px)`;
        } else if (progress.offsetWidth + rect.width < window.innerWidth && tooltip.style.left) {
    ...
    return (
        <div className={clsx(styles.slide)}>
            <div className={clsx(styles.topSlide)} ref={(el) => (slideRef.current.slideTop = el)}></div>
            <div>
                <span className={clsx(styles.remain)}>
                    Còn {remain}/{total} lần
                </span>
                <span className={clsx(styles.des)}>Bạn cần tìm kiếm một số từ 1 đến {number}</span>
            </div>
            <div
            ...
            >
                <div className={clsx(styles.progress)} ref={(el) => (slideRef.current.progress = el)}>
                    <div
                        className={clsx(styles.btn)}
                        ref={(el) => (slideRef.current.btn = el)}
                        onMouseDown={handleMouseMove}
                    >
                        <Tooltip ref={(el) => (slideRef.current.tooltip = el)} number={numberSet} />
                    </div>
                </div>
